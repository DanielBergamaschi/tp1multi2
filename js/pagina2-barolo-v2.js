...
function draw() {
  ...
  //image(barolo_ascii_oculto, 0, 0, width, height);
...
function dividir_imagenes() {
  ...
  for (let i = 0; i < 120; i++) {
    ...
    let ry = floor(random(filas));

    let recorte = palacio_barolo_fachada.get(
      rx * w,
      ry * h,
      w,
      h
    ...
    pedazos.push({
      ...
      targetAlpha: random(30, 180),
      opacidad: random(0, 100),

      velocidadFade: random(0.2, 1.2),

      apareciendo: random() > 0.5,

      espera: int(random(30, 200)),
      contador: int(random(200))
    });
  }
}
...
function dibujar_pedazos() {
  ...
  for (let i = 0; i < pedazos.length; i++) {
    ...
    if (p.apareciendo) {
      ...
      p.opacidad += p.velocidadFade;

      if (p.opacidad >= p.targetAlpha) {

        p.apareciendo = false;

        p.espera = int(random(40, 160));
      }

    } else {

      // FADE OUT suave
      p.opacidad -= p.velocidadFade * 0.6;

      // cuando desaparece
      if (p.opacidad <= 0) {

        // reaparece en otro lugar
        p.posX = random(-20, width);
        p.posY = random(-20, height);

        // tamaños más sutiles
        let nuevoTam = random(35, 95);

        p.ancho = nuevoTam;
        p.alto = nuevoTam * 1.3;

        p.targetAlpha = random(40, 190);

        // espera distinta para cada uno
        if (p.contador % p.espera == 0) {
          p.apareciendo = true;
        }
      }
    }
